...
- `DEV_CLOUDFRONT_TFVARS`: `env/dev/cloudfront/terraform.tfvars` 내용
...
`Terraform Apply Dev`는 `main` push 또는 수동 실행 시 `infra -> ecr -> addon -> cloudfront` 순서로 apply한 뒤 `scripts/post-apply-dev.sh`를 실행한다.
