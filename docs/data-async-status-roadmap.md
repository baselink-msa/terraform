...
채팅 컨텍스트가 바뀌거나 새 채팅방에서 작업을 이어가야 할 때는 먼저 `docs/project-continuity-handoff.md`를 확인합니다. 해당 문서는 사용자의 담당 역할, 작업 진행 방식, 최근 완료 작업, 현재 진행 중인 Kafka 개인 프로젝트, 남은 작업 우선순위를 인수인계용으로 정리합니다.
...
- seat-lock-service의 좌석 잠금 성공/실패/해제 이벤트 Kafka 발행 — 1차 구현 완료
- Valkey TTL 만료 기반 `SEAT_LOCK_EXPIRED` 이벤트 발행 구조 검토

2026-06-29 기준 Capacity Advisor 리포트에는 CloudWatch 기반 Valkey 상태가 1차 반영되었습니다. 현재 리포트는 Valkey `EngineCPUUtilization`, `DatabaseMemoryUsagePercentage`, `Evictions`, `ReplicationLag`를 조회해 `HEALTHY`, `CPU_HIGH`, `MEMORY_HIGH`, `EVICTIONS_DETECTED`, `REPLICATION_LAG`, `UNKNOWN`으로 분류합니다. 다만 좌석 잠금 event 자체를 Kafka로 수집하는 단계는 아직 남아 있습니다.
...
- Ansible 레포에 추가된 `capacity-advisor-flow.js`를 기준으로 부하테스트 검증 계획을 Terraform 문서로 정리
- `docs/load-test-validation-plan.md` 추가
- RDS, 대기열 자동 감속, SQS, Valkey, Kafka, S3/Athena, Capacity Advisor를 부하테스트에서 어떻게 검증할지 기준 정리
- RDS Proxy와 Read Replica 도입 여부를 부하테스트 결과로 판단하기 위한 조건 정리
- Capacity Advisor 추천값이 지나치게 낮게 나오는 경우의 해석 기준과 보정 방향 정리
- 부하테스트 결과 기록 템플릿 추가
- 발표/멘토 설명용 내 담당 파트 전체 요약 문서 `docs/my-part-presentation-outline.md` 추가
- PPT 슬라이드 구성, 발표 멘트, 캡처 후보, 예상 질문 답변 정리

현재 상태:

- 부하테스트 시나리오는 준비되어 있으나, `ansible/inventory.ini`가 없어 실제 원격 k6 실행은 대기 중
- 부하테스트 담당자에게 EC2 public IP, SSH user, private key 경로를 받아야 실제 실행 가능
- 팀 발표 자료는 각자 담당 파트를 PPT로 만든 뒤 합칠 예정이며, 내 파트는 `docs/my-part-presentation-outline.md`를 기준으로 작성 예정

다음 작업:

1. 부하테스트 EC2 inventory 확보
2. `capacity-advisor-flow.js` smoke 실행
3. baseline/load 단계 실행
4. Kafka -> S3 sink 실행
5. Capacity Advisor 결과와 AWS/Grafana 지표를 `docs/load-test-validation-plan.md` 템플릿에 기록
6. `docs/my-part-presentation-outline.md`를 바탕으로 실제 PPT 슬라이드 초안 작성
...
## 2026-06-29 seat-lock Kafka E2E 검증 업데이트

완료:

- `seat-lock-service`가 `reservation.lifecycle.events`로 좌석 잠금 이벤트를 발행하는 경로를 dev 환경에서 검증했다.
- Kafka S3 sink runner가 `seat-lock-service` 이벤트 5건을 S3 event lake에 적재했다.
- Glue/Athena `ticket_events` projection에 seat-lock 및 capacity signal 이벤트 타입을 추가했다.
- Athena에서 `SEAT_LOCK_REQUESTED`, `SEAT_LOCKED`, `SEAT_LOCK_FAILED`, `SEAT_UNLOCKED`가 조회되는 것을 확인했다.
...
- Kafka 개인 프로젝트가 대기열/예매 이벤트뿐 아니라 Valkey 기반 좌석 잠금 계층까지 포괄하게 되었다.
- 운영자는 나중에 좌석 잠금 성공률, 실패율, 해제 흐름을 Athena와 Capacity Advisor 리포트에서 함께 볼 수 있다.
- 심사위원 관점에서는 Kafka를 특정 알고리즘 보조 도구가 아니라 서비스 전반의 이벤트 스트리밍/관측성 인프라로 확장했다는 점을 설명할 수 있다.

남은 필수 작업:

1. 실제 부하테스트를 통해 RDS connection, SQS backlog, Kafka 이벤트 수, Capacity Advisor 추천값을 함께 검증한다.
2. 발표용으로 AWS 콘솔, Slack report, GitHub Actions, Athena 결과, S3 partition 화면을 정리한다.
3. 전체 프로젝트 발표 흐름에 맞춰 내 담당 파트 요약 문서를 최종 다듬는다.

선택 고도화:

1. `infra.audit.events`에 SQS/Worker backlog, DLQ, Kafka sink 실행 결과를 이벤트 이력으로 적재한다.
2. seat-lock 이벤트 기반 성공률/실패율/해제율 섹션을 Capacity Advisor 리포트에 추가한다.
3. `ADMISSION_THROTTLE_APPLIED`, SQS DLQ 발생 시 즉시 Slack 알림을 붙인다.
4. 예매 오픈 30분 전과 예매 진행 중 5분마다 Capacity Advisor를 자동 실행하는 운영 트리거를 추가한다.

## 2026-06-29 Capacity Advisor seat-lock/infra audit 확장

구현:

- Capacity Advisor JSON/Markdown 리포트에 좌석 잠금 이벤트 상태 섹션을 추가했다.
- Slack Capacity Advisor 메시지에도 좌석 잠금 이벤트 상태를 표시하도록 확장했다.
- `SEAT_LOCK_REQUESTED`, `SEAT_LOCKED`, `SEAT_LOCK_FAILED`, `SEAT_UNLOCKED` 기준으로 요청 수, 성공 수, 실패 수, 해제 수, 성공률, 실패율, 해제율을 계산한다.
- Kafka/SQS 파이프라인 자체 상태를 event lake에 남기기 위해 infra audit event type을 허용 목록에 추가했다.
- Kafka S3 sink 완료를 `KAFKA_S3_SINK_COMPLETED`로 기록하는 `--emit-audit-event` 옵션을 추가했다.
- SQS worker 상태를 `SQS_WORKER_STATUS_RECORDED`, `SQS_BACKLOG_DETECTED`, `SQS_DLQ_DETECTED`로 기록하는 `tools/record_sqs_worker_audit.py`를 추가했다.

검증:

- 단위 테스트 48개 통과
- 실제 dev Athena 기준 seat-lock summary 생성 확인
- Slack dry-run payload에 좌석 잠금 이벤트 상태 섹션 표시 확인
- Kafka S3 sink audit event dry-run 확인
- SQS worker audit recorder dry-run 확인

의미:

- Capacity Advisor가 단순히 안전 입장량만 추천하는 도구에서, 좌석 잠금/Valkey/SQS/Kafka까지 함께 보여주는 운영 리포트로 확장됐다.
- Kafka 개인 프로젝트가 대기열 알고리즘 보조가 아니라 서비스 전반의 이벤트 관측성 플랫폼이라는 설명이 더 강해졌다.
- 발표에서는 “운영자가 Slack에서 입장량 추천, DB 상태, SQS 상태, Valkey 상태, Kafka 상태, 좌석 잠금 이벤트 상태를 함께 보고 의사결정한다”고 설명할 수 있다.

추가 검증:

- Capacity Advisor Slack Report 수동 실행에서 `좌석 잠금 이벤트 상태` 섹션 표시를 확인했다.
- Slack 메시지에서 seat-lock 상태는 `COMPETITION_DETECTED`, 요청 2건, 성공 1건, 실패 1건, 해제 1건으로 표시됐다.
- `record_sqs_worker_audit.py`로 `SQS_WORKER_STATUS_RECORDED` 이벤트를 실제 S3/Athena event lake에 적재했다.
- `kafka_s3_sink.py --emit-audit-event`로 `KAFKA_S3_SINK_COMPLETED` 이벤트를 실제 S3/Athena event lake에 적재했다.
- Athena에서 두 audit event가 각각 1건씩 조회되는 것을 확인했다.
- audit event 반영 후 Capacity Advisor의 Kafka pipeline health에서 `sink_completed_events=1`을 확인했다.
- Capacity Advisor Slack Report 재실행 결과 Slack 메시지에서도 `KAFKA_S3_SINK_COMPLETED=1`, `SQS_WORKER_STATUS_RECORDED=1`, `sink completed 1` 표시를 확인했다.
