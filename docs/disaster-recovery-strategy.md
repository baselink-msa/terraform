...
| RDS PostgreSQL | Multi-AZ 활성화, backup retention 7일 설정 대기/적용 대상, PITR 전략 수립 | DB AZ 장애 대비는 좋음 |
...
| AWS Backup | 아직 별도 중앙 백업 정책 없음 | 다음 구현 대상 |
| Cross-Region DR | 아직 별도 DR 리전 구성 없음 | 전략 수립 필요 |
...
- RDS PITR이 활성화되면 특정 시점으로 새 DB를 복원할 수 있습니다.
...
- Terraform/GitOps/ECR 이미지/DB 백업을 이용해 DR 리전에 복구하는 전략이 필요합니다.

추천 전략:

- 비용과 구현 난이도를 고려해 `Pilot Light` 전략을 기본으로 합니다.
- 데이터는 다른 리전에 백업 복사본을 보관합니다.
...
| 대상 | 목표 RPO | 목표 RTO | 전략 |
| --- | ---: | ---: | --- |
| RDS PostgreSQL | 5분 이내 | 30~60분 | Multi-AZ + PITR + AWS Backup snapshot |
...
- RDS 백업 정책을 Terraform으로 관리합니다.
- backup vault를 생성합니다.
- 태그 기반 backup selection을 사용합니다.
- RDS continuous backup과 snapshot backup을 함께 사용합니다.
- 필요한 경우 cross-region copy를 설정합니다.

권장 backup plan:

| Rule | 대상 | 주기 | 보존 | 목적 |
| --- | --- | --- | --- | --- |
| continuous-rds | RDS | continuous | 7일 | PITR |
| daily-rds-snapshot | RDS | daily | 7~14일 | 일 단위 복구/검증 |
| weekly-rds-copy | RDS snapshot | weekly | 30일 | DR 리전 보관 |
...
1. `modules/backup` 또는 `env/dev/backup` 레이어 설계
2. backup vault 생성
3. backup plan 생성
4. RDS 태그 기반 selection 추가
5. IAM role 설정
6. 필요 시 DR 리전 copy action 추가
7. on-demand backup과 restore 테스트
...
- 이 문서를 팀과 리뷰합니다.
- 서비스별 RPO/RTO를 확정합니다.
- dev와 운영 가정의 차이를 명확히 합니다.

### 2단계: RDS 보호 강화

- deletion protection 추가
- final snapshot 활성화
- AWS Backup backup vault/plan 추가
- PITR 복구 리허설

### 3단계: Valkey/SQS 안정성 보강

- Valkey snapshot retention 1일 이상 검토
- Valkey transit encryption 검토
- SQS 원본 큐 retention/long polling/visibility timeout 검토
- SQS SSE 명시
...
- S3 frontend versioning/replication
- DR tfvars 작성
