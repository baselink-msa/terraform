...
- 네트워크 Pilot Light: Terraform 구현·배포 대기
...
따라서 현재 목표와 운영 모델은 Pilot Light가 맞지만, 네트워크 배포와 도쿄 RDS 복원 리허설까지 완료해야 리전 복구 경로 전체가 검증됩니다.

Pilot Light 구성:

- DR 리전 VPC/Subnet Terraform 구현·배포 대기
...
- 도쿄 VPC, subnet, DB subnet group, security group — Terraform 구현·배포 대기
