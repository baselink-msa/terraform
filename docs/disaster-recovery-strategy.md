...
| Cross-Region DR | 서울 단일 리전, 교차 리전 복사와 DR 인프라는 미구현 | 도쿄 Pilot Light 구현 예정 |
...
| daily-rds-copy | 다음 구현 | RDS recovery point | daily | 14일 예정 | 도쿄 리전 장애 복구 |
...
1. Backup/Restore job 실패 EventBridge 알림
2. RDS native PITR 복원 리허설
3. 복원 DB를 사용하는 임시 backend smoke test
4. 도쿄 리전 backup vault와 daily copy action
...
- 백업/DR 문서와 실제 구성 동기화
- Backup/Restore 실패 알림
- RDS PITR 복원 리허설
- 복원 DB endpoint를 이용한 backend smoke test
- 실제 측정 RPO/RTO와 복구 증거 기록

### P1: 도쿄 Pilot Light

- 도쿄 backup vault와 RDS cross-region copy
- ECR cross-region replication
- DR 리전 Terraform 입력값과 최소 인프라 plan
- secret/config 복구 목록과 endpoint 전환 Runbook
