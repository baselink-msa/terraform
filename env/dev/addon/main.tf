...
resource "kubectl_manifest" "backend_config" {
  yaml_body = yamlencode({
    ...
    data = {
      ...
      KAFKA_ENABLED                                           = "true"
      KAFKA_BOOTSTRAP_SERVERS                                 = data.terraform_remote_state.infra.outputs.kafka_event_streaming_bootstrap_brokers_sasl_iam
...
# Robusta Helm chart 설치 시 serviceAccount.name: robusta-runner로 맞춰야 함
#==============================================================================
data "aws_iam_policy_document" "robusta_assume" {
  statement {
    ...
    condition {
      ...
      variable = "${local.oidc_url_bare}:sub"
      values = [
        "system:serviceaccount:monitoring:robusta-stack-runner-service-account",
        "system:serviceaccount:monitoring:robusta-stack-holmes-service-account"
      ]
    }
